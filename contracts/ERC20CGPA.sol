...
contract ERC20CGPA is Context, IERC20CGPA, IERC20Metadata {
    ...
    uint8 private immutable _MAX_USER_TYPE_COUNT; 
     ...
     * @param maxUserTypeCount_ to specify maximum user types count, for example 2 for having (Student, Committee Member)
     */
     
    constructor(string memory name_, string memory symbol_, uint8 scale_, uint8 maxUserTypeCount_) {
        _name = name_;
        _symbol = symbol_;
        MAX_PER_USER = scale_ * 10 ** decimals(); // we multiply to count for decimal points
        _MAX_USER_TYPE_COUNT = maxUserTypeCount_;
    }

    function _setUserType(address account, uint8 userType) internal virtual {
        _checkUserType(userType);
        _userData[account] = userData;
    }

    function _checkUserType(uint8 userType) private {
        require(userType > 0 && userType <= _MAX_USER_TYPE_COUNT, "Invalid User Type");
    }
    ...
    function _setCGPA(address account, UserData userData) internal virtual {
